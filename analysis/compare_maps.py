...
from apply_map import apply_map, myprint
...
def main(varnames, outputfile, testfiles, cntlfiles,
         t_indices=(None, None), mapfiles=None, percentile=5, verbose=False, **kwargs):
    #
    ...
    #
    if verbose: myprint('Select time indices...')
    datasets = [ds.mean(dim='time', keep_attrs=True) if t_index is None else
                ds.isel(time=t_index) for ds, t_index in zip(datasets, t_indices)]
    #
    ...
    #plots = [axes[i].pcolormesh(lons[i], lats[i], data_arrays[i]) for i in
    #         range(len(data_arrays))]
    #
    # Annotate maps
    #
